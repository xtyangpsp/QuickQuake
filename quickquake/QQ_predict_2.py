...
def read_args():
    ...
    parser.add_argument("--min_p_prob", default=0.8, type=float, help="Probability threshold for P pick")
    parser.add_argument("--min_s_prob", default=0.8, type=float, help="Probability threshold for S pick")
...
def pred_fn(args, data_reader, figure_dir=None, prob_dir=None, log_dir=None):
    ...
    with tf.compat.v1.Session(config=sess_config) as sess:
        ...
        for _ in tqdm(range(0, data_reader.num_data, batch_size), desc="Pred"):
            ...
            # Reemplaza este bloque dentro de pred_fn:

            if args.plot_figure:
                # Decodifica nombres de archivo y estaciones
                names = [
                ...
                # Construye etiquetas "<sin_extensión>_<estación>"
                labels = []
                for name, st in zip(names, stations):
                    base = os.path.splitext(name)[0]  # quita ".mseed"
                    labels.append(f"{base}_{st}")

                # Lanza el plotting usando esas etiquetas
                pool.starmap(
                    partial(plot_waveform, figure_dir=figure_dir),
                    zip(X_batch, pred_batch, labels),
                )
